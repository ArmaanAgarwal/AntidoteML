...
def test_make_pool_picks_by_execution(cfg):
    assert isinstance(make_pool(cfg, make_splits(), {}), InProcessPool)
    with pytest.raises(ValueError):
        make_pool(Config(name="unit", execution="threads"), make_splits(), {})
...
def test_only_the_active_workers_run(monkeypatch, cfg, global_flat):
    ...
    pool = InProcessPool(cfg, make_splits(), {})
...
def test_the_update_does_not_depend_on_who_else_ran(cfg, global_flat):
    alone = InProcessPool(cfg, make_splits(), {}).run_round(global_flat, 4, [2])
    crowd = InProcessPool(cfg, make_splits(), {}).run_round(global_flat, 4, [0, 1, 2, 3, 4])
...
def test_two_pools_with_the_same_seed_agree(cfg, global_flat):
    a = InProcessPool(cfg, make_splits(), {}).run_round(global_flat, 1, [0, 1, 2])
    b = InProcessPool(cfg, make_splits(), {}).run_round(global_flat, 1, [0, 1, 2])
    for wid in (0, 1, 2):
        assert torch.equal(a[wid], b[wid])


def test_every_update_is_a_flat_float32_cpu_tensor(cfg, global_flat):
    results = InProcessPool(cfg, make_splits(), {}).run_round(global_flat, 1, [0, 1])
...
def test_each_worker_gets_its_own_spec_and_nobody_elses(monkeypatch, cfg, global_flat):
    ...
    InProcessPool(cfg, make_splits(), specs).run_round(global_flat, 1, [0, 1])
...
def test_the_round_number_reaches_the_worker(monkeypatch, cfg, global_flat):
    calls = spy_on_worker_calls(monkeypatch)
    InProcessPool(cfg, make_splits(), {}).run_round(global_flat, 12, [0])
    assert calls[0]["round"] == 12


def test_train_images_truncates_every_split():
    splits = make_splits(num_workers=5, n=40)
...
def test_the_pool_applies_train_images_at_init(cfg, global_flat):
    cfg.train_images = 10
    pool = InProcessPool(cfg, make_splits(n=40), {})
